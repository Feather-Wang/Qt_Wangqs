...
/*
WordEngine类：
是操作word文档得类，目前主要封装了创建表格，在表格中插入数据，插入图片，插入字符串等简单功能。
*/

test_doc::test_doc(QWidget *parent) : QWidget(parent)
{
    WordEngine engine;
    ...
    if( list.empty() )
        table = engine.insertTable("wangqs_tables", 3, 4);
    else
        table = engine.insertTable("wangqs_tables", 3, 4, list);
    /*设置表格得第4列得宽度为200*/
    engine.setColumnWidth(table, 4, 200);
    /*在表格得2行1列位置，写入tables21*/
    engine.SetTableCellString(table, 2, 1, "table21");
    engine.SetTableCellString(table, 3, 4, "table34");

    /*在标签wangqs_picture处，插入图片d:/w.jpg*/
    engine.replacePic(tr("wangqs_picture"), tr("d:/w.jpg"));
    /*在标签wangqs_text处，插入图片insert_text*/
    engine.replaceText(tr("wangqs_text"), tr("insert_text"));

    engine.save(tr("d:/a1.doc"));
    engine.close();
}
...
bool WordEngine::Open(QString sFile, bool bVisible)
{
     ...
     bool bFlag = m_pWord->setControl( "kwps.Application" );
     if(!bFlag)
     {
        return false;
     }
